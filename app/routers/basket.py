...
from app.schemas.basket import BasketResultSchema, BasketRunRequest
...
@router.get("/{job_id}/results", response_model=BasketResultSchema)
async def get_basket_results(
    job_id: str,
    current_user: CurrentUser = Depends(get_current_user),
    db_client: Client = Depends(get_db_client),
) -> BasketResultSchema:
    job = db.get_job(db_client, job_id, current_user.org_id)
    if job is None or job["module"] != "basket":
        raise AppError("not_found", "Job not found", 404)
    if job["status"] != "succeeded":
        raise AppError("conflict", f"Job is not finished (status={job['status']})", 409)
    return BasketResultSchema(**job["result_summary"])
